...
model = CompactMLP(input_dim, DROPOUT).to(DEVICE)
...
for epoch in range(1, EPOCHS + 1):
    ...
    if val_loss < best_val_loss - 1e-5:
        ...
        epochs_no_improve = 0
        os.makedirs("mlp-outputs", exist_ok=True)
        torch.save(model.state_dict(), "mlp-outputs/best_model.pt")
    else:
        epochs_no_improve += 1
...
plt.grid(True)
plt.tight_layout()
plt.savefig("mlp-outputs/mlp_loss_curve.png", dpi=300)
plt.show()
plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig("mlp-outputs/mlp_accuracy_curve.png", dpi=300)
plt.show()
plt.close()

# test evaluation
model.load_state_dict(torch.load("mlp-outputs/best_model.pt"))
model.eval()
...
metrics_df.to_csv("mlp-outputs/test_metrics.csv", index=False)
...
plt.ylabel("Actual")
plt.title("Confusion Matrix - Compact MLP")
plt.tight_layout()
plt.savefig("mlp-outputs/mlp_confusion_matrix.png", dpi=300)
plt.show()
plt.close()
